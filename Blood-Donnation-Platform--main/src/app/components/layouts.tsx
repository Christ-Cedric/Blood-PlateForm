...
import { getHospitalNotificationsApi } from "../api";
...
function LogoutButton({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl border transition-all duration-200 group mb-2 ${
        dark
...
export function DonorLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { icon: Home, labelKey: "nav.donor.home", path: "/donor/feed", label: "Accueil" },
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F8] flex flex-col md:flex-row max-w-[1440px] mx-auto relative">
      ...
      <header className="md:hidden flex items-center justify-between px-5 py-3.5 bg-white border-b border-[#EBEBEB] sticky top-0 z-50 shadow-[0_1px_8px_rgba(0,0,0,0.06)]">
        <Link to="/donor/feed" className="flex items-center gap-2 group">
      ...
      <aside className="hidden md:flex flex-col w-[240px] bg-white border-r border-[#EBEBEB] h-screen sticky top-0 shadow-[1px_0_0_#EBEBEB]">
        ...
        <div className="px-6 pt-6 pb-2">
          <Link to="/donor/feed" className="flex items-center gap-2.5 group">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all ${
                isActive ? "text-[#CC0000]" : "text-[#AAAAAA]"
              }`}
            >
              <div
                className={`p-1.5 rounded-xl transition-all ${
                  isActive ? "bg-[#CC0000]/10" : ""
                }`}
              >
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell className="w-5 h-5 text-[#444444]" />
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 w-4 h-4 bg-[#CC0000] rounded-full border-2 border-white text-[9px] text-white font-bold flex items-center justify-center">
            {unreadCount}
          </span>
        )}
      </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-[#EBEBEB] z-[110] overflow-hidden"
            >
              <div className="px-5 py-3 border-b border-[#F0F0F0] bg-[#FAFAFA] flex items-center justify-between">
                <span className="text-[13px] font-bold text-[#111111]">Notifications</span>
              ...
              <div className="max-h-[350px] overflow-y-auto">
                {notifications.length === 0 ? (
                  <div className="p-10 text-center">
                     ...
                     <p className="text-xs text-[#AAAAAA]">Aucune notification</p>
                  </div>
                ) : (
                  notifications.map((n) => (
                    <div key={n._id} className="p-4 border-b border-[#F8F8F8] hover:bg-[#F9F9F9] transition-colors">
                      <p className="text-[13px] text-[#333333] leading-relaxed">{n.message}</p>
                      <p className="text-[10px] text-[#AAAAAA] mt-1.5 font-medium">
              ...
              <div className="p-3 bg-[#FBFBFB] text-center border-t border-[#F0F0F0]">
                 <button onClick={() => setIsOpen(false)} className="text-[11px] font-bold text-[#888888] hover:text-[#555555]">Fermer</button>
              </div>
            </motion.div>
          </>
        )}
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              onClick={onLinkClick}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 group ${
                isActive ? "bg-[#1E1E1E] text-white" : "text-[#666666] hover:bg-[#1A1A1A] hover:text-[#AAAAAA]"
              }`}
              style={{ fontFamily: "'DM Sans', sans-serif" }}
            >
              <item.icon
      ...
      <div className="px-3 mb-5">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-[#1A1A1A] border border-[#252525] mb-2">
          ...
          <div className="flex-1 truncate">
            ...
            <p className="text-xs text-[#555555]" style={{ fontFamily: "'DM Sans', sans-serif" }}>Administrateur système</p>
          </div>
        </div>
        <LogoutButton dark />
      </div>
    ...
    <div className="min-h-screen bg-[#F7F7F8] flex flex-col md:flex-row max-w-[1440px] mx-auto relative overflow-x-hidden">
      ...
      <main className="flex-1 bg-[#F7F7F8] min-h-screen">
        ...
        <div className="hidden md:flex bg-white border-b border-[#EBEBEB] px-8 py-4 items-center justify-between sticky top-0 z-10 shadow-[0_1px_6px_rgba(0,0,0,0.04)]">
          <div>
            <p className="text-xs text-[#888888] font-medium uppercase tracking-wider" style={{ fontFamily: "'DM Sans', sans-serif" }}>Administration</p>
            <p className="text-sm font-semibold text-[#111111]" style={{ fontFamily: "'DM Sans', sans-serif" }}>Vue nationale — SangVie</p>
          </div>

    ...
    try {
      if (userType === "donor") {
        ...
        const data = await registerUserApi({
          ...
          lieuResidence: formData.location || "Non renseigné",
        ...
        await registerHospitalApi({
          ...
          region: formData.region || "Non renseignée",
          localisation: formData.location || "Non renseignée",
        });
        setIsSuccess(true);
      }
    } catch (err: any) {
        ...
        <div className="flex-1 flex items-center justify-center p-6 bg-[#F7F7F8]">
          ...
          >
            ...
            <p className="text-[15px] text-[#666666] mb-8 leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Votre demande d'inscription pour <strong>{formData.hospitalName}</strong> a été transmise avec succès. Nos équipes vérifieront votre agrément sous 24h à 48h.
            </p>
            <Link to="/login">
        ...
        <div className="hidden lg:flex flex-col justify-between w-[40%] relative overflow-hidden p-12">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-[38px] font-bold text-white leading-tight mb-4 drop-shadow-lg" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Rejoignez<br /><span className="text-[#FF4444]">SangVie</span>
            </h2>
            <p className="text-white/80 text-[14px] leading-relaxed max-w-[280px]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Donneurs ou établissements de santé, rejoignez notre réseau et contribuez à sauver des vies.
            </p>
          </div>

          <div className="relative z-10 space-y-3">
            {["Alertes d'urgence en temps réel", "Suivi de l'impact de vos dons", "Réseau d'hôpitaux certifiés", "Interface simple et intuitive"].map((benefit, i) => (
                ...
                >
                  {userType === "hospital" ? (
                    <>
                      <div>
                        ...
                        <Input type="text" placeholder="Centre Hospitalier Universitaire" value={formData.hospitalName} onChange={(e) => handleChange("hospitalName", e.target.value)} required />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-[#333333] mb-1.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                          {t("auth.register.hospitalId")}
                          <span className="ml-1 text-[11px] font-normal text-[#AAAAAA]">(Numéro d'agrément)</span>
                        </label>
                        <Input type="text" placeholder="HOS-2024-001" value={formData.hospitalId} onChange={(e) => handleChange("hospitalId", e.target.value)} required />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-[#333333] mb-1.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                          Téléphone / Contact
                        </label>
                        <Input type="tel" placeholder="+226 XX XX XX XX" value={formData.phone} onChange={(e) => handleChange("phone", e.target.value)} required />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-[#333333] mb-1.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                          {t("auth.register.hospitalEmail")}
                        </label>
                        <Input type="email" placeholder="contact@hopital.com" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} required />
                      </div>
                    </>
                  ) : (
                    <>
                      <div>
                        ...
                        <Input type="text" placeholder="Jean Dupont" value={formData.fullName} onChange={(e) => handleChange("fullName", e.target.value)} required />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-[#333333] mb-1.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      ...
                      <div>
                        ...
                        <Input type="email" placeholder="votre@email.com" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} required />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Input type={showPassword ? "text" : "password"} placeholder="Créer un mot de passe (min. 6 caractères)" value={formData.password} onChange={(e) => handleChange("password", e.target.value)} required />
                  ...
                  <div>
                    ...
                    <Input type={showPassword ? "text" : "password"} placeholder="Confirmer le mot de passe" value={formData.confirmPassword} onChange={(e) => handleChange("confirmPassword", e.target.value)} required />
                  </div>
                </motion.div>
              </AnimatePresence>
              ...
              {userType === "hospital" && (
                ...
                >
                  ...
                  <p className="text-[12px] text-[#D4720B] leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                    Votre compte sera activé après vérification de votre agrément par l'équipe SangVie.
                  </p>
                </motion.div>
              )}

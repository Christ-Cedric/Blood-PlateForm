...
export function DonorFeed() {
  ...
  const handleRespond = async () => {
    ...
    try {
      ...
      setSuccessMessage("Merci ! Votre réponse a été transmise à l'hôpital. Veuillez vous y rendre dès que possible.");
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] mt-0.5" style={{ fontFamily: "'DM Sans', sans-serif", color: isActive ? "#1A7A3F" : "#AAAAAA" }}>
                {isActive ? "✓ Vous êtes visible pour les urgences" : "Vous ne recevrez pas d'alertes"}
              </p>
            </div>
          </div>
          {/* Toggle */}
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[20px] font-bold text-[#0A0A0A]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Urgences autour de vous
            </h2>
            <p className="text-[12px] text-[#AAAAAA] mt-0.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              {isLoading ? "Recherche en cours..." : `${requests.length} demandes actives en ce moment`}
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 mb-24 md:mb-8">
          {isLoading && (
             <div className="flex flex-col items-center justify-center py-20 gap-4">
               ...
               <p className="text-[#888888] text-sm">Chargement des urgences...</p>
             </div>
          )}
          ...
          {!isLoading && error && (
            <div className="text-center py-10">
              ...
              <Button variant="secondary" onClick={fetchRequests}>Réessayer</Button>
            </div>
          )}
          ...
          {!isLoading && !error && requests.length === 0 && (
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-[#E0E0E0]">
              ...
              <p className="text-[#888888] font-medium">Aucune urgence signalée pour le moment.</p>
            </div>
          )}

          {!isLoading && requests.map((demande, idx) => {
            const config = URGENCY_CONFIG[demande.niveauUrgence] || URGENCY_CONFIG.normal;
            return (
              <motion.div
                key={demande._id}
              ...
              >
                <div
                  ...
                  className={`bg-white rounded-2xl border border-[#EBEBEB] overflow-hidden cursor-pointer hover:shadow-[0_6px_24px_rgba(0,0,0,0.10)] hover:-translate-y-0.5 transition-all duration-200 ${config.cardBorder}`}
                >
                  {/* Card header */}
                  ...
                  <div className="border-t border-[#F0F0F0] px-4 py-3 flex items-center justify-between bg-[#FAFAFA]">
                    <span className="text-[12px] font-semibold text-[#AAAAAA]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      {demande.quantitePoches} {demande.quantitePoches > 1 ? "poches" : "poche"} requise{demande.quantitePoches > 1 ? "s" : ""}
                    </span>
                    <button
              ...
              >
                ...
                <div className="overflow-y-auto flex-1 p-6">
                  ...
                  <div className="flex flex-col items-center mb-6 text-center">
                    <div
                      className={`w-20 h-20 rounded-2xl ${URGENCY_CONFIG[selectedRequest.niveauUrgence]?.groupBg ?? "bg-[#CC0000]"} flex items-center justify-center mb-3 shadow-lg`}
                    >
                      <span className="text-white font-bold text-3xl" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                        {selectedRequest.groupeSanguin}
                      </span>
                    </div>
                    <Badge variant={URGENCY_CONFIG[selectedRequest.niveauUrgence]?.badgeVariant ?? "critical"} className="mb-2">
                      {URGENCY_CONFIG[selectedRequest.niveauUrgence]?.label || "Urgence"}
                    </Badge>
                    <h2 className="text-[20px] font-bold text-[#0A0A0A]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                  ...
                  <div className="bg-[#F7F7F8] rounded-2xl p-4 mb-4">
                    ...
                    <p className="text-[14px] text-[#333333] leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      {selectedRequest.description || "Aucun détail supplémentaire fourni."}
                    </p>
                  </div>
                  ...
                  <div className="mb-6">
                    <label className="block text-[11px] font-bold text-[#AAAAAA] uppercase tracking-wider mb-2">
                       Message pour l&apos;hôpital (optionnel)
                    </label>
                    <textarea
                      value={message}
                      onChange={(e) => setMessage(e.target.value)}
                      placeholder="Ex: Je viendrai dans 30 minutes..."
                ...
                <div className="p-4 border-t border-[#F0F0F0] flex gap-3">
                  ...
                  >
                    {isResponding ? <Loader2 className="w-4 h-4 animate-spin" /> : "Je peux donner"}
                  </Button>
                </div>
              </motion.div>
            </div>

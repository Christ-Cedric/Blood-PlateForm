...
export function DonorHistory() {
  ...
  const firstDonationDate = completedDonations.length > 0 
    ? new Date(completedDonations[completedDonations.length - 1].dateDon).toLocaleDateString('fr-FR', { month: 'short', year: 'numeric' })
    : "Aucun";
    ...
    <DonorLayout>
      <div className="p-4 md:p-6 max-w-2xl mx-auto">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              ...
              <p className="text-white/60 text-[13px] mt-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                Grâce à {completedDonations.length} dons validés
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-[11px] text-white/50 mt-1.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>Groupe sanguin</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            { icon: Award, label: "Dons totaux", value: completedDonations.length.toString(), color: "text-[#D4720B]", bg: "bg-[#FFF8F0]" },
            { icon: Heart, label: "Vies sauvées", value: totalLives.toString(), color: "text-[#CC0000]", bg: "bg-[#FFF0F0]" },
            { icon: Calendar, label: "Premier don", value: firstDonationDate, color: "text-[#1A7A3F]", bg: "bg-[#F0FFF4]" },
          ].map((s, i) => (
            <motion.div
              key={i}
          ...
          <div className="space-y-3">
            {isLoading ? (
              <div className="py-20 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-[#888888] text-sm">Chargement de votre historique...</p>
              </div>
            ) : donations.length === 0 ? (
              <div className="py-20 text-center bg-white rounded-3xl border border-dashed border-[#E0E0E0]">
                ...
                <p className="text-[#888888] font-medium">Vous n'avez pas encore effectué de don.</p>
              </div>
            ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] font-bold text-[#0A0A0A] truncate" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                        {donation.hospitalId?.nom || donation.lieuDon}
                      </p>
                      <div className="flex items-center gap-3 mt-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[11px] text-[#AAAAAA]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                            {new Date(donation.dateDon).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </span>
                        </div>
                    ...
                    <div className="text-right flex-shrink-0">
                      {donation.statut === 'complete' ? (
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <Badge variant="pending" className="text-[10px]">En attente</Badge>
                          <span className="text-[10px] text-[#AAAAAA]">Prévu</span>
                        </div>
                      )}
                    </div>
                  </div>

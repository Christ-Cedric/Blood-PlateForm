...
export function HospitalDashboard() {
  ...
  const handleCreateRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("Demande publiée avec succès ! Les donneurs compatibles ont été notifiés.");
  ...
  const handleConfirmDonation = async (donationId: string) => {
    try {
      ...
      setSuccessMessage("Don confirmé avec succès ! Le donneur a été crédité.");
      setTimeout(() => setSuccessMessage(""), 5000);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-[28px] md:text-[34px] font-bold text-[#0A0A0A] leading-tight" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            Bonjour, {user?.nom || "Hôpital"} 👋
          </h1>
          <p className="text-[14px] text-[#888888] mt-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            {requests.filter(r => r.statut === "en_attente").length} demandes actives · {new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
        </div>
            ...
            <table className="w-full" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              <thead>
                <tr className="border-b border-[#F0F0F0]">
                  {["Groupe", "Quantité", "Urgence", "Statut", "Date", ""].map((h, i) => (
                  ...
                  <tr key={req._id} className="border-b border-[#F8F8F8] hover:bg-[#FAFAFA] transition-colors">
                    <td className="px-5 py-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-[13px] ${
                        req.niveauUrgence === "critique" ? "bg-[#CC0000]" : req.niveauUrgence === "urgent" ? "bg-[#D4720B]" : "bg-[#1A7A3F]"
                      }`}>
                        {req.groupeSanguin}
                      </div>
                    </td>
                    <td className="px-5 py-4 text-[14px] font-semibold text-[#111111]">
                      {req.quantitePoches} poche{req.quantitePoches > 1 ? "s" : ""}
                    </td>
                    <td className="px-5 py-4">
                      <Badge variant={req.niveauUrgence === "critique" ? "critical" : req.niveauUrgence === "urgent" ? "moderate" : "active"}>
                        {req.niveauUrgence === "critique" ? "Critique" : req.niveauUrgence === "urgent" ? "Urgent" : "Normal"}
                      </Badge>
                    </td>
                    <td className="px-5 py-4">
                      <Badge variant={req.statut === "en_attente" ? "pending" : req.statut === "satisfait" ? "success" : "critical"}>
                        {req.statut === "en_attente" ? "En cours" : req.statut === "satisfait" ? "Clôturée" : "Annulée"}
                      </Badge>
                    </td>
                    <td className="px-5 py-4 text-[13px] text-[#AAAAAA]">{formatTime(req.createdAt)}</td>
              ...
              <form onSubmit={handleCreateRequest} className="flex-1 overflow-y-auto">
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {BLOOD_GROUPS.map((g) => (
                        <button
                          ...
                          className={`h-11 rounded-xl border-2 font-bold text-[14px] transition-all duration-150 active:scale-95 ${
                            bloodGroup === g
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {[
                        { value: "critique", label: "Critique", sub: "Urgence vitale — besoin immédiat", color: "#CC0000", bg: "bg-[#FFF0F0]" },
                        { value: "urgent", label: "Urgent", sub: "Besoin sous 24h à 48h", color: "#D4720B", bg: "bg-[#FFF8F0]" },
                        { value: "normal", label: "Normal", sub: "Intervention programmée", color: "#1A7A3F", bg: "bg-[#F0FFF4]" },
                      ].map((level) => (
                        <button
                          key={level.value}
                          type="button"
                          onClick={() => setUrgency(level.value as any)}
                          className={`p-3.5 rounded-xl border-2 flex items-center justify-between text-left transition-all duration-150 ${
                            urgency === level.value
                        ...
                        >
                          ...
                          <div
                            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                              urgency === level.value ? "border-current" : "border-[#D0D0D0]"
                            }`}
                            style={{ borderColor: urgency === level.value ? level.color : undefined }}
                          >
                            {urgency === level.value && (
                  ...
                  <div>
                    <label className="block text-[13px] font-bold text-[#333333] mb-2 uppercase tracking-wide" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      ...
                      <span className="text-[#BBBBBB] font-normal normal-case">(optionnel)</span>
                    </label>
                    <textarea
                      value={description}
                ...
                <div className="sticky bottom-0 px-6 py-4 border-t border-[#F0F0F0] bg-white flex gap-3">
                  ...
                  <Button type="submit" className="flex-1" disabled={isSubmitting}>
                    {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Soumettre la demande"}
                  </Button>
                </div>
              </form>
            </motion.div>
        ...
        {selectedRequestForDetails && (
          <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-0 md:p-4">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative z-10 bg-white w-full max-w-md rounded-t-3xl md:rounded-3xl p-0 flex flex-col shadow-2xl overflow-hidden max-h-[80vh]">
              <div className="px-6 py-5 border-b border-[#F0F0F0] flex items-center justify-between">
                <div>
                  <h2 className="text-[17px] font-extrabold text-[#0A0A0A]">Détails des réponses</h2>
                  <p className="text-[11px] text-[#AAAAAA]">Groupe {selectedRequestForDetails.groupeSanguin} · {responses.length} volontaires</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[250px]">
                {isLoadingResponses && (
                  <div className="flex flex-col items-center justify-center py-12 gap-3">
                    <Loader2 className="w-8 h-8 text-[#CC0000] animate-spin" />
                    <p className="text-xs text-[#AAAAAA]">Recherche des donneurs...</p>
                  </div>
                )}
                ...
                {!isLoadingResponses && responses.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-12 text-center opacity-40">
                    ...
                    <p className="text-sm font-medium">En attente de donneurs volontaires.</p>
                  </div>
                )}
                    ...
                    <div className="flex items-center gap-2">
                      {res.statut === "confirme" ? (
                        ...
                        <button 
                          ...
                          title="Confirmer que le don a été effectué"
                        >
              ...
              <div className="p-4 bg-[#FAFAFA] border-t border-[#F0F0F0]">
                 <Button onClick={() => setSelectedRequestForDetails(null)} className="w-full bg-[#0A0A0A]">Fermer</Button>
              </div>
            </motion.div>
          </div>
        )}

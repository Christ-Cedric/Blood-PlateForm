...
const URGENCY_CONFIG = {
  critique: { label: "Urgence vitale", bg: "bg-[#CC0000]", light: "bg-[#FFF0F0]", border: "border-[#CC0000]/30", text: "text-[#CC0000]" },
  urgent: { label: "Urgent", bg: "bg-[#D4720B]", light: "bg-[#FFF8F0]", border: "border-[#D4720B]/30", text: "text-[#D4720B]" },
  normal: { label: "Normal", bg: "bg-[#555555]", light: "bg-[#F5F5F5]", border: "border-[#EBEBEB]", text: "text-[#555555]" },
};
...
export function HospitalRequests() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("Demande publiée avec succès !");
  ...
  const handleUpdateStatus = async (id: string, statut: "satisfait" | "annule") => {
    try {
      ...
      setSuccessMessage(`Demande ${statut === "satisfait" ? "clôturée" : "annulée"}.`);
      setTimeout(() => setSuccessMessage(""), 4000);
      fetchRequests();
    } catch (error) {
      console.error(error);
  ...
  const handleConfirmDonation = async (donationId: string) => {
    try {
      ...
      toast.success("Le don a été confirmé avec succès !");
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {isLoading && (
          <div className="py-20 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-[#888888] text-sm">Chargement de vos demandes...</p>
          </div>
        )}
          ...
          <div className="mb-12">
            ...
            {active.length === 0 && (
              <div className="py-12 bg-white rounded-3xl border border-dashed border-[#E0E0E0] text-center">
                <p className="text-[#AAAAAA] text-sm">Aucune demande active en cours.</p>
              </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {active.map((req, idx) => {
                const cfg = URGENCY_CONFIG[req.niveauUrgence] || URGENCY_CONFIG.normal;
                return (
                  <motion.div
                    key={req._id}
                    ...
                    <div className="p-5 flex-1">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-[14px] font-bold text-[#0A0A0A] leading-tight line-clamp-1">
                              {req.description || "Besoin en sang"}
                            </p>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        <div className={`rounded-xl p-3 ${cfg.light} flex items-center gap-2`}>
                          ...
                          <div>
                            <p className="text-[9px] font-bold text-[#AAAAAA] uppercase">Quantité</p>
                            <p className={`text-[13px] font-bold ${cfg.text}`}>{req.quantitePoches} poches</p>
                          </div>
                        </div>
                        <div className="rounded-xl p-3 bg-[#F0FFF4] flex items-center gap-2 border border-[#1A7A3F]/10">
                          ...
                          <div>
                            <p className="text-[9px] font-bold text-[#AAAAAA] uppercase">Type</p>
                            <p className="text-[13px] font-bold text-[#1A7A3F]">Bénévolat</p>
                          </div>
                        </div>
                      </div>
              ...
              {closed.slice(0, 5).map((req) => (
                <div key={req._id} className="bg-white rounded-xl border border-[#F0F0F0] p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[13px] font-bold text-[#333333] line-clamp-1">{req.description || "Demande de sang"}</p>
                  ...
                  <Badge variant={req.statut === "satisfait" ? "success" : "critical"}>
                    {req.statut === "satisfait" ? "Satisfaite" : "Annulée"}
                  </Badge>
                </div>
              ))}
            </div>
        ...
        {showNewRequest && (
          <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center p-0 md:p-4">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative z-10 bg-white w-full max-w-lg rounded-t-3xl md:rounded-3xl p-6 md:p-8 flex flex-col shadow-2xl overflow-hidden max-h-[95vh]">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-[20px] font-extrabold text-[#0A0A0A]">Créer une urgence</h2>
                  <p className="text-[12px] text-[#AAAAAA] mt-0.5">Les donneurs compatibles recevront une alerte</p>
                </div>
              ...
              <form onSubmit={handleCreate} className="space-y-6 overflow-y-auto">
                <div>
                  <label className="block text-[12px] font-bold text-[#333333] uppercase mb-3">Groupe sanguin</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12px] font-bold text-[#333333] uppercase mb-3">Quantité (Poches)</label>
                    <Input type="number" min="1" value={form.quantity} onChange={(e) => setForm({...form, quantity: e.target.value})} required className="h-12" />
                  </div>
                  <div>
                    <label className="block text-[12px] font-bold text-[#333333] uppercase mb-3">Urgence</label>
                ...
                <div>
                  <label className="block text-[12px] font-bold text-[#333333] uppercase mb-3">Détails de l'urgence</label>
                  <textarea value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} required className="w-full h-24 rounded-2xl border-2 border-[#EBEBEB] p-4 text-sm focus:outline-none focus:border-[#CC0000] resize-none" placeholder="Précisez la situation médicale..." />
                </div>

                <Button type="submit" className="w-full h-14 rounded-2xl text-[16px] font-bold bg-[#CC0000]" disabled={isSubmitting}>
                  {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : "Diffuser l'alerte"}
                </Button>
              </form>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center p-0 md:p-4">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative z-10 bg-white w-full max-w-lg rounded-t-3xl md:rounded-3xl p-0 flex flex-col shadow-2xl overflow-hidden max-h-[85vh]">
              <div className="px-6 py-5 border-b border-[#F0F0F0] flex items-center justify-between bg-[#FBFBFB]">
                <div>
                  <h2 className="text-[17px] font-extrabold text-[#0A0A0A]">Réponses ({responses.length})</h2>
                  <p className="text-[11px] text-[#AAAAAA]">Donneurs volontaires pour le groupe {selectedRequest.groupeSanguin}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[300px]">
                {isLoadingResponses && (
                  <div className="flex flex-col items-center justify-center py-12 gap-3">
                    <Loader2 className="w-8 h-8 text-[#CC0000] animate-spin" />
                    <p className="text-xs text-[#AAAAAA]">Chargement des volontaires...</p>
                  </div>
                )}
                ...
                {!isLoadingResponses && responses.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-12 text-center opacity-40">
                    ...
                    <p className="text-sm font-medium">Aucun donneur n'a encore répondu.</p>
                  </div>
                )}
              ...
              <div className="p-4 border-t border-[#F0F0F0]">
                 <Button onClick={() => setSelectedRequest(null)} className="w-full h-11 rounded-xl bg-[#0A0A0A]">Fermer</Button>
              </div>
            </motion.div>
          </div>
        )}

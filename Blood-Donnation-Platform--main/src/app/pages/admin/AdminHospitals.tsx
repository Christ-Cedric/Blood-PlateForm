        ...
        <div className="space-y-4">
          {isLoading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-[#888888]">Chargement de la liste...</p>
            </div>
              ...
              <Card key={hospital._id} className="hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <Badge variant={hospital.verified ? "success" : "pending"}>
                    {hospital.verified ? (
                      <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Vérifié</span>
                    ) : (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4 py-3 border-t border-b border-[#F0F0F0]">
                  <div>
                    <Typography.Small className="text-[#888888] mb-0.5">Agrément</Typography.Small>
                    <Typography.Body className="font-semibold text-sm">{hospital.numeroAgrement}</Typography.Body>
                  </div>
                  <div>
                    <Typography.Small className="text-[#888888] mb-0.5">Contact</Typography.Small>
                  ...
                  <div>
                    <Typography.Small className="text-[#888888] mb-0.5">Inscrit le</Typography.Small>
                ...
                <div className="flex gap-2">
                  {!hospital.verified ? (
                    <>
                      ...
                      >
                        {processingId === hospital._id ? <Loader2 className="w-3 h-3 animate-spin" /> : "Approuver"}
                      </Button>
                      <Button 
                        variant="secondary" 
            ...
            <div className="flex flex-col">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 opacity-90">
                      <Badge variant={selectedHospital.verified ? "success" : "pending"} className="bg-white/20 border-white/30 text-white">
                        {selectedHospital.verified ? "Vérifié" : "En attente"}
                      </Badge>
                      <span className="text-xs font-medium">Inscrit le {new Date(selectedHospital.createdAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 bg-white grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-widest block mb-2">Informations de contact</label>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-widest block mb-2">Documents administratifs</label>
                    <div className="p-4 rounded-2xl bg-[#F9F9FB] border border-[#F0F0F2]">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <p className="text-xs font-bold text-[#111111]">N° d'Agrément</p>
                  ...
                  {!selectedHospital.verified && (
                    <div className="p-4 rounded-2xl bg-[#FFF8F0] border border-[#FFE8CC]">
                      <p className="text-[11px] font-bold text-[#D4720B] mb-2">ACTION REQUISE</p>
                      <p className="text-[12px] text-[#A35200] leading-relaxed mb-4">
                        Ce compte attend votre validation. Veuillez vérifier manuellement le numéro d'agrément auprès des autorités compétentes.
                      </p>
                      <div className="flex gap-2">
                        <Button 
                          className="flex-1 h-9 text-xs" 

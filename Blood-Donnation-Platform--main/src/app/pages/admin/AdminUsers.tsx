...
export function AdminUsers() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Supprimer ce compte donneur ? Cette action est irréversible.")) return;
    try {
    ...
    <AdminLayout>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {[
            { label: "Total donneurs", value: users.length.toString(), icon: User, bg: "bg-[#F5F5FF]", color: "text-[#5B5BD6]" },
            { label: "Actifs", value: users.filter(u => u.statutDonneur === "actif").length.toString(), icon: TrendingUp, bg: "bg-[#F0FFF4]", color: "text-[#1A7A3F]" },
            { label: "Dons totaux", value: totalDonations.toString(), icon: Heart, bg: "bg-[#FFF0F0]", color: "text-[#CC0000]" },
          ].map((s, i) => (
            <motion.div
              key={i}
          ...
          <div className="divide-y divide-[#F8F8F8]">
            {isLoading ? (
              <div className="p-12 flex flex-col items-center justify-center gap-3">
                ...
                <p className="text-sm text-[#AAAAAA]">Chargement des donneurs...</p>
              </div>
            ) : filtered.length === 0 ? (
                  ...
                  <div className="hidden md:flex items-center gap-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[13px] font-semibold text-[#333333]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                        {user.donationsCount || 0} dons
                      </span>
                    </div>
                  </div>

                   <Badge variant={user.statutDonneur === "actif" ? "success" : "inactive"}>
                    {user.statutDonneur === "actif" ? "Actif" : "Inactif"}
                  </Badge>
            ...
            <div className="flex flex-col">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <Badge variant={selectedUser.statutDonneur === "actif" ? "success" : "inactive"} className="bg-white/10 border-white/20 text-white">
                        {selectedUser.statutDonneur === "actif" ? "Donneur Actif" : "Inactif"}
                      </Badge>
                      <span className="text-xs text-white/60 font-medium">Membre depuis {new Date(selectedUser.createdAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 border-b border-[#F0F0F0]">
                <div className="p-4 text-center border-r border-[#F0F0F0]">
                  <p className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-wider mb-1">Dons</p>
                  <p className="text-xl font-bold text-[#CC0000]">{selectedUser.donationsCount || 0}</p>
                </div>
                <div className="p-4 text-center border-r border-[#F0F0F0]">
                  <p className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-wider mb-1">Rang</p>
                  <p className="text-xl font-bold text-[#111111]">Or</p>
                </div>
                <div className="p-4 text-center">
                  <p className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-wider mb-1">Vies sauvées</p>
              ...
              <div className="p-8 bg-white space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-widest block">Contact</label>
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-widest block">Localisation</label>
                ...
                <div className="pt-4 border-t border-[#F5F5F5]">
                  <label className="text-[10px] font-bold text-[#AAAAAA] uppercase tracking-widest block mb-4">Activité récente</label>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 rounded-xl bg-[#F9F9FB] border border-[#F0F0F2]">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-semibold text-[#333333]">Dernier don</p>
                      </div>
                      <p className="text-xs font-medium text-[#888888]">Il y a 2 mois</p>
                    </div>
              ...
              <div className="p-6 border-t border-[#F0F0F0] bg-[#FAFAFA] flex items-center justify-between">
                ...
                <Button variant="secondary" onClick={() => setSelectedUser(null)} className="rounded-xl px-8"> Fermer </Button>
              </div>
            </div>
          )}
        </DialogContent>

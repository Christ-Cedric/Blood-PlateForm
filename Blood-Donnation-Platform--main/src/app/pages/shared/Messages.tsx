        ...
        <div className={`w-full md:w-80 border-r border-[#F0F0F0] flex flex-col bg-[#FBFBFB] transition-transform duration-300 ${selectedConv ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoadingConvs ? (
              ...
              <div className="p-8 text-center text-[#AAAAAA] text-sm">Aucune discussion</div>
            ) : (
              conversations.map((c) => (
                <div 
                  key={c.otherId}
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#CC0000]/10 flex items-center justify-center text-[#CC0000] flex-shrink-0">
                      {c.otherType === 'Hospital' ? <Building2 className="w-5 h-5" /> : <User className="w-5 h-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm truncate font-['DM_Sans'] text-[#111111]">{c.otherType === 'Hospital' ? 'Hôpital' : 'Donneur'}</p>
          ...
          {selectedConv ? (
            <>
              ...
              <div className="p-4 border-b border-[#F0F0F0] bg-[#FBFBFB] flex items-center gap-3 relative shadow-sm z-10">
                ...
                <div className="w-9 h-9 rounded-full bg-[#CC0000]/10 flex items-center justify-center text-[#CC0000]">
                  {selectedConv.otherType === 'Hospital' ? <Building2 className="w-4 h-4" /> : <User className="w-4 h-4" />}
                </div>
                <div className="flex flex-col">
                  <h3 className="font-bold text-[15px] font-['DM_Sans'] text-[#111111] leading-tight">
                    {selectedConv.otherType === 'Hospital' ? 'Hôpital' : 'Donneur'}
                  </h3>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-[#AAAAAA]">
              ...
              <h3 className="text-xl font-bold text-[#333333] mb-2 font-['DM_Sans']">Vos messages</h3>
              <p className="text-[14px] max-w-sm">Sélectionnez une discussion dans la liste pour commencer à échanger avec un hôpital ou un donneur.</p>
            </div>
          )}
        </div>
      </div>
